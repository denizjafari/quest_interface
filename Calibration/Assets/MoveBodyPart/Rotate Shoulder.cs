...
public class RotateObject : MonoBehaviour
{
    ...
    void Update()
    {
        if (connectedTcpClient != null)
        {
            ...
            if (stream.DataAvailable)
            {
                ...
                RotateGameObject(armData.shoulder_rotation_right);
                UpdateMinMax(armData.shoulder_rotation_right);
            }
        }
    }
    ...
    private void UpdateMinMax(float shoulderRotation)
    {
        if (shoulderRotation > max)
        {
            max = shoulderRotation;
        }

        if (shoulderRotation < min)
        {
            min = shoulderRotation;
        }
    }
