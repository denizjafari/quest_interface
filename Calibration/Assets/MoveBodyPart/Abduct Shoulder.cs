...
public class Abduct_Shoulder : MonoBehaviour
{
    ...
    void Update()
    {
        if (connectedTcpClient != null)
        {
            ...
            if (stream.DataAvailable)
            {
                ...
                RotateGameObject(armData.shoulder_abduction_right);
                UpdateMinMax(armData.shoulder_abduction_right);
            }
        }
    }
    ...
    private void UpdateMinMax(float shoulderAbduction)
    {
        if (shoulderAbduction > max)
        {
            max = shoulderAbduction;
        }

        if (shoulderAbduction < min)
        {
            min = shoulderAbduction;
        }
    }

    private void OnDisable()
    {
        string path = "../../game_config/shoulder_abduction_ROM.json";
